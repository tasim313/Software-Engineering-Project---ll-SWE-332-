...
from .models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, \
    LeaveReportTeacher, Teachers, FeedBackTeacher, CustomUser, Courses, NotificationTeacher, \
...
def teacher_home(request):
    ...
    return render(request, "teacher_templates/teacher_home_template.html",
                  {"students_count": students_count, "attendance_count": attendance_count,
                   "leave_count": leave_count, "subject_count": subject_count, "subject_list": subject_list,
                   "attendance_list": attendance_list, "student_list": student_list,
                   "present_list": student_list_attendance_present, "absent_list": student_list_attendance_absent})


def teacher_take_attendance(request):
    subjects = Subjects.objects.filter(teacher_id=request.user.id)
    session_years = SessionYearModel.object.all()
    return render(request, "teacher_templates/teacher_take_attendance.html", {"subjects": subjects,
...
def teacher_update_attendance(request):
    ...
    return render(request, "teacher_templates/teacher_update_attendance.html",
...
def teacher_apply_leave(request):
    ...
    return render(request, "teacher_templates/teacher_apply_leave.html", {"leave_data": leave_data})
...
def teacher_apply_leave_save(request):
    if request.method != "POST":
        ...
        teacher_obj = Teachers.objects.get(admin=request.user.id)
        try:
            leave_report = LeaveReportTeacher(teacher_id=teacher_obj, leave_date=leave_date,
                                              leave_message=leave_msg, leave_status=0)
            leave_report.save()
...
def teacher_feedback(request):
    ...
    return render(request, "teacher_templates/teacher_feedback.html", {"feedback_data": feedback_data})
...
def teacher_profile(request):
    ...
    return render(request, "teacher_templates/teacher_profile.html", {"user": user, "teacher": teacher})
...
@csrf_exempt
def teacher_fcmtoken_save(request):
...
def teacher_all_notification(request):
    teacher = Teachers.objects.get(admin=request.user.id)
    notifications = NotificationTeacher.objects.filter(teacher_id=teacher.id)
    return render(request, "teacher_templates/all_notification.html", {"notifications": notifications})


def teacher_add_result(request):
    subjects = Subjects.objects.filter(teacher_id=request.user.id)
    session_years = SessionYearModel.object.all()
    return render(request, "teacher_templates/teacher_add_result.html",
...
def start_live_classroom(request):
    subjects = Subjects.objects.filter(teacher_id=request.user.id)
    session_years = SessionYearModel.object.all()
    return render(request, "teacher_templates/teacher_live_classroom.html",
                  {"subjects": subjects, "session_years": session_years})
...
def start_live_classroom_process(request):
    ...
    if checks:
        ...
        teacher_obj = Teachers.objects.get(admin=request.user.id)
        onlineClass = OnlineClassRoom(room_name=roomname, room_pwd=room_pwd,
                                      subject=subject_obj, session_years=session_obj,
                                      started_by=teacher_obj, is_active=True)
        onlineClass.save()

    return render(request, "teacher_templates/live_class_room_start.html",
                  {"username": request.user.username, "password": room_pwd, "roomid": roomname,
                   "subject": subject_obj.subject_name, "session_year": session_obj})
